...
os.environ["DISC_MEM_INTENSIVE_OPT_EXPERIMENTAL"] = "false"
...
with torch.no_grad():
    ...
    # torch.jit.save(blade_model, 'bert_large_on.disc.pt')
    torch.jit.save(blade_model, 'bert_large_off.disc.pt')
    ...
    # blade_model = torch.jit.load('bert_large_on.disc.pt').cuda().eval()
    blade_model = torch.jit.load('bert_large_off.disc.pt').cuda().eval()
