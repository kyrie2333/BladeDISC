...
__global__ void colreduct1_32x32(float* data_in, float* data_out, int M, int N,
                                  int tile_size) {
...
__global__ void colreduct1_64x32(float* data_in, float* data_out, int M, int N,
                                  int tile_size) {
  int block_x = blockIdx.x % ((N + blockDim.x - 1) / blockDim.x);
  int block_y = blockIdx.x / ((N + blockDim.x - 1) / blockDim.x);
  int col = block_x * blockDim.x + threadIdx.x;
...
int main(int argc, char* argv[]) {
  ...
  colreduct1_32x32<<<grid, block>>>(d_matrix, d_result, M, N, TILE_SIZE);
  ...
  colreduct1_64x32<<<grid, block>>>(d_matrix, d_result, M, N, TILE_SIZE);
  CHECK(
