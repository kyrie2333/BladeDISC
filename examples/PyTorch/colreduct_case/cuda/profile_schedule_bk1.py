...
def gen_reduction_results1(file):
    results = {}
    blocksz =  [8, 16, 32, 64, 128, 256, 512, 1024]
    tile = [32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768]
            ...
            for BLOCK_X in blocksz:
                for TILE_SIZE in tile:
                    ...
                    if float(latency) > 0.000001:
                        results[(M, N)][(BLOCK_X, TILE_SIZE)] = latency
    print("Results", results)
    np.save("reduction1_results_a10.npy", results)

def gen_reduction_results2(file):
    results = {}
    blocksize = [(8, 16), (8, 32),  
                 (16, 8), (16, 16), (16, 32),  
                 (32, 8), (32, 16), (32, 32)
                ]
    for M in range(512, 32768 + 1, 512):
        for N in range(512, 32768 + 1, 512):
            results[(M, N)] = {}  
    ...
    print("Results", results)
    np.save("reduction2_results_a10.npy", results)

def specific_tuning():
    results1 = np.load('reduction1_results_a10.npy', allow_pickle=True).item()
    results2 = np.load('reduction2_results_a10.npy', allow_pickle=True).item()
    # ranges = [
    #     (512, 512, 1024, 1024),
    #     (1024, 1024, 2048, 2048),
    #     (2048, 2048, 2560, 2560),
    ...
    ranges = [
        ...
        (8192, 8192, 8192, 9216),
        (8192, 9216, 9216, 9216),
        (9216, 9216, 9216, 10240),
        (9216, 10240, 10240, 10240),
        (10240, 10240, 10240, 11264),
        (10240, 11264, 11264, 11264),
        (11264, 11264, 11264, 12288),
        (11264, 12288, 12288, 12288)
    ]
    schedules1 = [
        (8, 8),
        (16, 8),
...
gen_reduction_results1("colreduct1")
# gen_reduction_results2("colreduct2")
specific_tuning()
