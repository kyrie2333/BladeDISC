            ...
            for BLOCK_X in blocksz:
                for TILE_SIZE in tile:
                    ...
                    if float(latency) > 0.000001:
                        results[(M, N)][(BLOCK_X, TILE_SIZE)] = latency
    print("Results", results)
    np.save("reduction1_results_t4.npy", results)
...
def gen_reduction_results2(file):
    ...
    print("Results", results)
    np.save("reduction2_results_t4.npy", results)

def specific_tuning():
    results1 = np.load('reduction1_results_t4.npy', allow_pickle=True).item()
    results2 = np.load('reduction2_results_t4.npy', allow_pickle=True).item()
    # ranges = [
    #     (512, 512, 1024, 1024),
    #     (1024, 1024, 2048, 2048),
    #     (2048, 2048, 2560, 2560),
