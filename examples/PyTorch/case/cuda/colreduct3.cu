...
void check_result(float* host_ref, float* gpu_ref) {
  ...
  bool match = 1;
  for (int i = 0; i < N; i++) {
    if (abs((host_ref[i] - gpu_ref[i] / (WARMUPS + ITERS)) / host_ref[i]) > epsilon) {
      match = 0;
...
__global__ void column_reduce(float* data_in, float* data_out) {
  // shared memory size : 40KB = 40 * 1024 bytes = 10240 fp32 elements
  ...
  for (int row = 0; row < M; row += blockDim.y) {
    int globalRow = row + threadIdx.y;
    sdata[threadIdx.x][threadIdx.y] = data_in[globalRow * N + col];
  }
  __syncthreads();

  // row shuffle (width may > 32)
  for (int stride = blockDim.y / 2; stride > 0; stride >>= 1) {
    if (threadIdx.y < stride) {
      sdata[threadIdx.x][threadIdx.y] += sdata[threadIdx.x][threadIdx.y + stride];
    }
    __syncthreads();
  }
  ...
  int main() {
    ...
    CHECK(cudaMemcpy(result, d_result, N * sizeof(float),
